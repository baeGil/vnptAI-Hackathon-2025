...
Consolidates answer-related logic from vnpt-ai-sample:
- Extraction from LLM responses using explicit patterns
...
def extract_answer(response: str, max_choices: int = 4) -> str | None:
    ...
    # Fallback: Find first single letter that matches valid choices
    simple_match = re.search(r'\b([A-Z])\b', response)
    if simple_match:
...
def validate_answer(answer: str, num_choices: int) -> tuple:
...
def normalize_answer(
    ...
    if answer is None:
        if question_id:
            print(f"[Warning] No answer extracted for {question_id}, defaulting to {default}")
        return default
    ...
    if not is_valid:
        if question_id:
            print(f"[Warning] Invalid answer '{answer}' for {question_id}, defaulting to {default}")
        return default
